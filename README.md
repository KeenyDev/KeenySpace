...
Agents consuming KeenySpace over MCP need the usage contract in their system prompt or
`CLAUDE.md`: pages are read-only, the append-only WAL via `append_log` is the only write
surface, `compile` is a separate step before new pages appear, every tool call is scoped
to one `workspace` slug, and navigation is `[[wikilink]]` traversal rather than search.

The full text lives in [docs/keenyspace-agent-prompt.md](docs/keenyspace-agent-prompt.md).
Copy that file into every project that connects to a KeenySpace workspace.
...
| [CONTRIBUTING.md](CONTRIBUTING.md) | Scope and non-goals, development setup, running the server from source, the checks CI runs |
...
[CONTRIBUTING.md](CONTRIBUTING.md) is the canonical developer guide: the DCO sign-off
requirement, what is in and out of scope for v1, how to bring up the development database,
how to run the server from source without Docker, and the exact commands CI runs.
