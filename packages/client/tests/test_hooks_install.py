...
def test_install_creates_file_when_absent(
    ...
    session_start_commands = [
        obj["command"] for g in hooks["SessionStart"] for obj in g.get("hooks", [])
    ]
...
def test_install_preserves_foreign_hooks_and_other_keys(
    ...
    foreign_group = {"hooks": [{"type": "command", "command": "bash /x/gsd-session-state.sh"}]}
    ...
    foreign_present = any(
        any(obj.get("command") == "bash /x/gsd-session-state.sh" for obj in g.get("hooks", []))
        for g in session_start
    )
    assert foreign_present

    ours_present = any(
        any(obj.get("command", "").startswith("keenyspace hook ") for obj in g.get("hooks", []))
        for g in session_start
    )
    assert ours_present
...
def test_install_idempotent_no_duplicates(
    ...
    ours_count = sum(
        1
        for g in session_start
        if any(obj.get("command", "").startswith(OURS_PREFIX) for obj in g.get("hooks", []))
    )
    assert ours_count == 2
...
def test_install_upgrades_stale_ours_entry(
    ...
    settings_path.write_text(json.dumps({"hooks": {"SessionStart": [stale_group]}}))
...
def test_uninstall_removes_only_ours(
    ...
    settings_path.parent.mkdir(parents=True, exist_ok=True)
    foreign_group = {"hooks": [{"type": "command", "command": "bash /x/gsd-session-state.sh"}]}
    settings_path.write_text(json.dumps({"hooks": {"SessionStart": [foreign_group]}}))
...
def test_status_installed_not_partial(
    ...
    partial_settings = {"hooks": {"SessionStart": [KEENYSPACE_HOOKS["SessionStart"][0]]}}
