...
pytestmark = pytest.mark.skipif(not PG_URL, reason="postgres unavailable; KEENYSPACE_DB__URL not set")
...
def _alembic(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["uv", "run", "alembic", *args],
        cwd=SERVER_DIR, env={**os.environ, "KEENYSPACE_DB__URL": PG_URL or ""},
...
def test_0005_refuses_rows_that_violate_new_constraints() -> None:
    ...
    _sql([(_INSERT_WORKSPACE, {"uuid": uuid4(), "slug": "bad", "status": "archived", "archived_at": None})])
...
def test_0005_downgrade_drops_intent_only_cursors_and_reupgrades() -> None:
    ...
    _sql([
        (_INSERT_WORKSPACE, {"uuid": committed_ws, "slug": "a", "status": "active", "archived_at": None}),
        (_INSERT_WORKSPACE, {"uuid": intent_only_ws, "slug": "b", "status": "active", "archived_at": None}),
        (
...
def test_0005_cursor_rows_follow_workspace_deletion() -> None:
    ...
    ws = uuid4()
    rows = _sql([
        (_INSERT_WORKSPACE, {"uuid": ws, "slug": "gone", "status": "active", "archived_at": None}),
        (
            "INSERT INTO compile_cursors (workspace_uuid, last_wal_id, last_compile_hash, updated_at) "
            "VALUES (:ws, 'A', 'h', now())",
            {"ws": ws},
