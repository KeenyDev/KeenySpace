...
- [x] **VERIFIED (backup phase live; destructive drill blockers fixed in 07-10 — final green CI run gates on next tag push)** — 07-04 Task 3 checkpoint live evidence: `keenyspace backup` produced a 2.2 MB tarball containing `manifest.json` + `pg_dump.sql` + 1612 fs_root entries; workspace count returned 3. The `deploy/scripts/backup-restore-drill.sh` script (commit 5cb04db) implements the full drill including the destructive `down -v` -> restore cycle with `COMPOSE_PROJECT_NAME=ks-drill` throwaway guard. The destructive half runs in `drill.yml` CI — 07-08 put the CLI on PATH and seeded a workspace, but introduced three further blockers: auth.json written at mode 0644 (triggers `AuthFileModeError`), `KEENYSPACE_SERVER_URL` never set (pydantic `ValidationError` before any HTTP call), and the script re-upped with the base compose only (losing the `blueprints-test` overlay after `down -v`, so reauth could not succeed). All three blockers were closed in 07-10: `chmod 600` added after each jq write, `KEENYSPACE_SERVER_URL` set in the drill step env, and `DRILL_COMPOSE_FILES` passed through so the overlay survives the wipe. The destructive CI cycle has not yet executed green; it will run on the next `v*` tag push. Release blocker fixed during this phase: `pg_dump` was missing from the Docker image until commit 899cc7b added `postgresql-client-17`.
