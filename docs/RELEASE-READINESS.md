...
- [x] **VERIFIED (backup phase live; destructive drill green 2026-09-14)** — 07-04 Task 3 checkpoint live evidence: `keenyspace backup` produced a 2.2 MB tarball containing `manifest.json` + `pg_dump.sql` + 1612 fs_root entries; workspace count returned 3. The `deploy/scripts/backup-restore-drill.sh` script (commit 5cb04db) implements the full drill including the destructive `down -v` -> restore cycle with `COMPOSE_PROJECT_NAME=ks-drill` throwaway guard. The destructive half runs in `drill.yml` CI — 07-08 put the CLI on PATH and seeded a workspace, but introduced three further blockers: auth.json written at mode 0644 (triggers `AuthFileModeError`), `KEENYSPACE_SERVER_URL` never set (pydantic `ValidationError` before any HTTP call), and the script re-upped with the base compose only (losing the `blueprints-test` overlay after `down -v`, so reauth could not succeed). All three blockers were closed in 07-10: `chmod 600` added after each jq write, `KEENYSPACE_SERVER_URL` set in the drill step env, and `DRILL_COMPOSE_FILES` passed through so the overlay survives the wipe. The destructive CI cycle executed green on 2026-09-14 (`drill.yml` run `34894775271`, dispatched manually on `main`): `Backup-restore drill PASSED - 1 workspace(s) intact`, with `proxy-test-caddy` and `proxy-test-nginx` green in the same run. That first live run exposed four defects on the unforced-restore path, all fixed in PR #6 — the endpoint deadlocked against its own open transaction while `psql` replayed a `--clean` dump; pooled connections kept asyncpg statement caches invalidated by that replay; restore cleared conflicting targets with `rmtree` and so died on the file-shaped `.image-sync.json`; and the drill script only re-authenticated before the restore, though replaying the dump revokes the key minted after `down -v`. The first three left a restore half-applied (database replaced, files not) and were invisible to the test suite, which only exercised `force=true`. Release blocker fixed during this phase: `pg_dump` was missing from the Docker image until commit 899cc7b added `postgresql-client-17`.
...
| REL-07 | Backup-restore drill executed and documented before release | `deploy/scripts/backup-restore-drill.sh`; backup phase verified live 2026-06-11 (2.2 MB tarball, 3 workspaces); destructive half green in `drill.yml` run `34894775271` on 2026-09-14 after the unforced-restore fixes in PR #6 | 07-08, 07-10, PR #6 | Complete |
